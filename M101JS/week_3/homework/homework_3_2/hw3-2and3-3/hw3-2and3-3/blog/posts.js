...
function PostsDAO(db) {
    ...
    this.insertEntry = function (title, body, tags, author, callback) {
        ...
        // now insert the post
        // hw3.2 TODO
        posts.save(post, function(err, saved){
          if(err) return callback(err, null);
          console.dir("Succesfully saved: " + saved + " document(s)")
          return callback(null, saved.permalink);
        });
    ...
    this.addComment = function(permalink, name, email, body, callback) {
        ...
        posts.findOne({ "permalink": permalink }, function(err, doc){
          if(err || doc === null) return callback(err,null);
          doc.comments.push(comment);
          posts.save(doc, function(err, saved) {
            if(err) throw err;
